...
def Phi_alpha(rho, phi, z):
    x = rho * np.cos(phi)
    y = rho * np.sin(phi)
    cartesian_point = np.array([x, y, z]).T @ CYLINDER_ROTATION + CYLINDER_CENTER
...
def compute_coefficients(m, n):
    ...
    def compute_integral_vec(integrand):
        return integrate.quad_vec(
            lambda z: integrate.dblquad(
                lambda phi, rho: integrand(z, phi, rho),
                0,
                2 * np.pi,
                lambda _: 0,
                lambda _: ALPHA * R_star,
            )[0],
            0,
            L,
        )[0]

    A_mn_integral = compute_integral_vec(integrand_A)
    B_mn_integral = compute_integral_vec(integrand_B)
