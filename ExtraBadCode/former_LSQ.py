...
n_n, n_m = 40, 40  # Truncation parameters
num_params = 4 * n_n * n_m
...
def print_fitted_parameters(fitted_params, n_n, n_m):
    ...
    for m in range(n_m):
        for n in range(1, n_n + 1):
            idx = m * n_n + (n - 1)
            print(f"  A[m={m}, n={n}] = {A[idx]:.6e}")

    print("Coefficients B:")
    for m in range(n_m):
        for n in range(1, n_n + 1):
            idx = m * n_n + (n - 1)
            print(f"  B[m={m}, n={n}] = {B[idx]:.6e}")

    print("Coefficients C:")
    for m in range(n_m):
        for n in range(1, n_n + 1):
            idx = m * n_n + (n - 1)
            print(f"  C[m={m}, n={n}] = {C[idx]:.6e}")

    print("Coefficients D:")
    for m in range(n_m):
        for n in range(1, n_n + 1):
            idx = m * n_n + (n - 1)
            print(f"  D[m={m}, n={n}] = {D[idx]:.6e}")
...
def plot_coefficient_matrices(A, B, C, D, n_n, n_m):
...
plot_coefficient_matrices(A, B, C, D, n_n, n_m)
