...
# Date: 2021-09-30
#
# TODO: 1) Fai accelerazioni 2) Fix units 3) Fix how surrogate takes in cartesian 4) Scaling? 5) constant density?
#########################################################################################################################
...
def prepare_linear_system_new(points, potentials, n_n, n_m):
    ...
    num_params = 6 * n_n * n_m  # Accounts for A, B, C, D, E, F coefficients
    ...
    for m in range(n_m):
        for n in range(1, n_n + 1):
            ...
            sinh_term_1 = np.sinh(k_mn / R * z)
            sinh_term_2 = np.sinh(k_mn / R * (L - z))

            bessel_i = BesselI(m, n * np.pi / L * rho)
            sin_term = np.sin(n * np.pi / L * z)

            # A_mn and B_mn terms
            A[:, idx] = sinh_term_1 * bessel_j * np.cos(m * phi)
            A[:, idx + 1] = sinh_term_1 * bessel_j * np.sin(m * phi)

            # C_mn and D_mn terms
            A[:, idx + 2] = sinh_term_2 * bessel_j * np.cos(m * phi)
            A[:, idx + 3] = sinh_term_2 * bessel_j * np.sin(m * phi)

            # E_mn and F_mn terms
            A[:, idx + 4] = sin_term * bessel_i * np.cos(m * phi)
            A[:, idx + 5] = sin_term * bessel_i * np.sin(m * phi)
...
n_n, n_m = 4, 4  # Truncation parameters
num_params = 6 * n_n * n_m  # Updated for six coefficients per term
...
def print_fitted_parameters(fitted_params, n_n, n_m):
    ...
    D = fitted_params[3 * n_m * n_n : 4 * n_m * n_n]
    E = fitted_params[4 * n_m * n_n : 5 * n_m * n_n]
    F = fitted_params[5 * n_m * n_n :]

    print("Fitted Parameters (in scientific notation):")
    for coeff, label in zip([A, B, C, D, E, F], ["A", "B", "C", "D", "E", "F"]):
        print(f"Coefficients {label}:")
        for m in range(n_m):
            for n in range(1, n_n + 1):
                idx = m * n_n + (n - 1)
                print(f"  {label}[m={m}, n={n}] = {coeff[idx]:.6e}")

    return A, B, C, D, E, F


# Function to plot coefficients in semilogarithmic scale with distinct colors
def plot_coefficients_semilogy(A, B, C, D, E, F, n_n, n_m):
    ...
    def scatter_coefficients(coefficients, label, color):
        for m in range(n_m):
            # Extract coefficients for a specific order m
            coeffs_m = coefficients[m * n_n : (m + 1) * n_n]
            # Scatter plot for each order n
            plt.scatter(
                [m] * n_n,
                np.abs(coeffs_m),
                color=color,
    ...
    # Scatter coefficients for A, B, C, D, E, F
    ...
    scatter_coefficients(E, "E", colors["E"])
    scatter_coefficients(F, "F", colors["F"])
...
def plot_coefficient_matrices(A, B, C, D, E, F, n_n, n_m):
    # Reshape coefficients into matrices of size (n_m, n_n)
    A_matrix = np.reshape(A, (n_m, n_n))
    B_matrix = np.reshape(B, (n_m, n_n))
    C_matrix = np.reshape(C, (n_m, n_n))
    D_matrix = np.reshape(D, (n_m, n_n))
    E_matrix = np.reshape(E, (n_m, n_n))
    F_matrix = np.reshape(F, (n_m, n_n))
...
# Call the functions
A, B, C, D, E, F = print_fitted_parameters(fitted_params, n_n, n_m)
plot_coefficients_semilogy(A, B, C, D, E, F, n_n, n_m)
plot_coefficient_matrices(A, B, C, D, E, F, n_n, n_m)
...
def calculate_acceleration_from_potential(points, fitted_params, n_n, n_m):
    ...
    acceleration_cylindrical = np.zeros_like(points)
    ...
    for m in range(n_m):
        for n in range(1, n_n + 1):
            ...
            dV_drho = -k_mn / R * exp_term * bessel_j * cos_m_phi * fitted_params[idx]
            dV_dphi = -m * exp_term * bessel_j * sin_m_phi * fitted_params[idx + 1]
            dV_dz = (
                -(n * np.pi / L)
                * sin_term
                * bessel_i
            ...
            acceleration_cylindrical[:, 1] += dV_dphi / rho  # angular
