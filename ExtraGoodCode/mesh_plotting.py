...
def plot_grid_2d(
    ...
    if plot_rectangle:
        rect = patches.Rectangle(
            (-1, -1), 2, 2, linewidth=1, edgecolor="r", facecolor="none"
        )
        ax.add_patch(rect)
...
def plot_quiver(
    ...
    ax.quiver(
        X, Y, U, V, angles="xy", linewidth=0.1, color="b", pivot="mid", units="xy"
    )
...
def plot_polygon(ax, vertices, coordinate):
    ...
    if ACTIVATE:
        ...
        polygon = patches.Polygon(
            xy, closed=True, linewidth=1, edgecolor="r", facecolor="none"
        )
