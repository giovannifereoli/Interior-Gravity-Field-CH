...
# Date: 2024-09-30
#
# TODO: 1) Fai accelerazioni, fit in acc but with err/a for high altitude 2) Fix units 6) recursive least sqaure?
#########################################################################################################################
...
for point in tqdm(cylinder_points, desc="Evaluating gravity at points"):
    ...
    results.append(
        {
            "point": point,
            "potential": potential,  # Add noise to the potential
...
# Prepare the matrix A and vector b for the linear least squares fitting
def prepare_linear_system(points, potentials, n_n, n_m):
    ...
    for m in range(n_m):
        for n in range(1, n_n + 1):
            # Normalization coefficient for spherical harmonics-like expansion
            ...
            # Apply normalization coefficient to each term
            A[:, idx] = norm_coeff * exp_term * bessel_j * np.cos(m * phi)
            A[:, idx + 1] = norm_coeff * exp_term * bessel_j * np.sin(m * phi)
            A[:, idx + 2] = norm_coeff * sin_term * bessel_i * np.cos(m * phi)
            A[:, idx + 3] = norm_coeff * sin_term * bessel_i * np.sin(m * phi)
            idx += 4
    return A, b
...
A, b = prepare_linear_system(points, potentials, n_n, n_m)
...
aug_b_noisy = np.hstack([b_noisy, np.zeros(M)])
...
# fitted_params = np.linalg.solve(A.T @ A + 1e-14 * np.eye(A.shape[1]), A.T @ b_noisy)
...
def calculate_acceleration_from_potential(points, fitted_params, n_n, n_m):
    ...
    acceleration_cylindrical = np.zeros((len(points), 3))
    ...
    for m in range(n_m):
        for n in range(1, n_n + 1):
            ...
            bessel_j_derivative = (
                (k_mn / R)
                * 0.5
                * (BesselJ(m - 1, k_mn / R * rho) - BesselJ(m + 1, k_mn / R * rho))
            )
            bessel_i = BesselI(m, n * np.pi / L * rho)
            bessel_i_derivative = (
                (n * np.pi / L)
            ...
            # Cylindrical components of the potential's gradient
            dV_drho = (
                bessel_j_derivative * exp_term * np.cos(m * phi) * fitted_params[idx]
                + bessel_j_derivative
                * exp_term
                * np.sin(m * phi)
                * fitted_params[idx + 1]
            ...
            dV_dphi = (
                -m * bessel_j * exp_term * np.sin(m * phi) * fitted_params[idx]
                + m * bessel_j * exp_term * np.cos(m * phi) * fitted_params[idx + 1]
                - m * bessel_i * sin_term * np.sin(m * phi) * fitted_params[idx + 2]
                + m * bessel_i * sin_term * np.cos(m * phi) * fitted_params[idx + 3]
            ) / rho
            dV_dz = (
                -k_mn / R * bessel_j * exp_term * np.cos(m * phi) * fitted_params[idx]
                - k_mn
                / R
                * bessel_j
                * exp_term
            ...
            acceleration_cylindrical[:, 0] += dV_drho
            acceleration_cylindrical[:, 1] += dV_dphi
            acceleration_cylindrical[:, 2] += dV_dz
            idx += 4
    ...
    # Transform back to the original Cartesian frame
    acceleration_cartesian = (
        acceleration_cartesian @ CYLINDER_ROTATION + CYLINDER_CENTER
    )
