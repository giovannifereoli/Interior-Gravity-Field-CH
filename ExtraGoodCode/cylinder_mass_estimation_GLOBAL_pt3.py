...
def sh_grad_basis(pts, Lmin, Lmax, Rref, h=1e-4):
    ...
    for k in range(3):
        ...
        d[k] = h
        out[:, k, :] = (
            sh_pot_basis(pts + d, Lmin, Lmax, Rref)
            - sh_pot_basis(pts - d, Lmin, Lmax, Rref)
        ) / (2 * h)
    return out
...
def delta_fisher(
    site, tm, V, F, ch_modes, Lmax, Rref, n_pts, seed=1, normalize=True, delta=1e-6
):
    """
    ...
    return dict(
        logdet=float(np.sum(np.log(lam + delta))),
        trace=float(np.trace(dF) / dF.shape[0]),
        n_obs=len(obs),
    )
...
def candidate_pool(
    V, F, tm, Rb, n_cand=140, brillouin_frac=0.80, min_vacuum=0.40, n_pts=180, seed=1
):
    """
    ...
    idx = P2.farthest_point_sample(
        surf, n_cand, start_idx=int(np.argmin(surf[:, 2])), seed=seed
    )
    cand = surf[idx]
    frac = np.array(
        [len(cylinder_at(s, tm, V, F, n_pts=n_pts)[1]) / n_pts for s in cand]
    )
...
def run(
    ...
    if verbose:
        ...
        print(SEP)
        print(
            f"  Brillouin R* = {Rb:.3f} LU | candidate sites {len(cand)}/{n_cand} "
            f"({n_cand-len(cand)} dropped as too buried) | {n_cyl} cylinders "
            f"| {n_m} anomalies"
        )
        print(
            f"  BULK: constant-density polyhedron, β̃ = 1 − Σβ = {beta_bulk:.3f} of M*"
        )
        for nm, p, fr in zip(names, P, f_true):
            print(
                f"    {nm:14s} p={np.round(p,3)}  β={fr:+.3f}  "
                f"|r|={np.linalg.norm(p):.2f}"
            )

    # ── the criterion maps, on the candidate pool ───────────────────────────
    P_kaula = kaula_covariance(Lmax_sh, K=kaula_K)
    P_track, obs_track = tracking_covariance(Lmax_sh, Rref, gap_lon=gap_lon, K=kaula_K)
    ...
    fps_idx = P2.farthest_point_sample(
        cand, n_cyl, start_idx=int(np.argmin(cand[:, 2]))
    )
    d_fps = min(
        np.linalg.norm(cand[i] - cand[j]) for i in fps_idx for j in fps_idx if i != j
    )
    min_sep = 0.5 * d_fps
    ...
    # Two criteria that select the same SET of sites are the same network, and
    # racing them is one experiment reported twice.  The order the greedy scan
    # emits them in differs, so compare sets, not sequences.
    _pk = list(picks)
    dupes = [
        (a, b)
        for i, a in enumerate(_pk)
        for b in _pk[i + 1 :]
        if set(picks[a].tolist()) == set(picks[b].tolist())
    ]
    ...
    if verbose:
        ...
        print(
            f"  eps |g-g_SH|            : {err_raw.min():.2e} … {err_raw.max():.2e} "
            f"({err_raw.max()/err_raw.min():.0f}× spread)"
        )
        print(
            f"  sigma_g (tracking P_SH) : {sig_trk.min():.2e} … {sig_trk.max():.2e} "
            f"({sig_trk.max()/sig_trk.min():.0f}× spread, "
            f"coverage gap at lon {gap_lon[0]:.0f}–{gap_lon[1]:.0f}°)"
        )
        print(
            f"  DeltaF orthogonal frac. : {sc_tr.min():.3f} … {sc_tr.max():.3f}  "
        ...
        for nm, s in (
            ("eps raw", err_raw),
            ("eps detrended", err_det),
            ("sigma_g Kaula", sig_kau),
            ("sigma_g tracking", sig_trk),
            ("sigma_g tr. detr.", sig_det),
            ("logdet DeltaF", sc_fis),
        ):
            print(f"    {nm:20s} rho = {spearmanr(s, rlog).statistic:+.3f}")
        print(
            "    ⇒ EVERY criterion taken literally is a near-perfect proxy for "
        ...
        print(
            f"    detrended sigma_g puts {in_gap.sum()}/{n_cyl} cylinders inside "
        ...
        for k, i in picks.items():
            ...
            print(
                f"    {k:16s} sites {np.sort(i)}  ({same}/{n_cyl} shared with FPS, "
                f"mean NN spacing {np.mean(d):.3f} LU)"
            )
        for a, b in dupes:
            print(
                f"    !! {a} and {b} selected the SAME {n_cyl} sites — they are one\n"
                f"       network, so their columns below are duplicates, not two "
    ...
    # ANALYTIC sigma, not a Monte-Carlo one.  The mass fit is linear, so the
    # posterior covariance is exactly (A^T W A)^-1; a 300-draw MC estimates that
    # same number with ~3% scatter, and the WORST-CASE column below is a max
    # over anomalies, which turns that scatter into a biased, noisy statistic —
    # the thing the strategies are ranked on.  beta~ = 1 - sum(beta) is derived,
    # so its variance is 1^T C 1; its recovered VALUE still comes from one
    # honest noisy fit.
    one = np.ones(len(P))
    sig_by_case, bulk_by_case = {}, {}
    for k, b in cases.items():
        C = np.linalg.inv(G.fisher_masses(b))
        sig_by_case[k] = np.sqrt(np.diag(C))
        b_hat = G.ls_fit_once(b, f_true, np.random.default_rng(5))
        bulk_by_case[k] = (float(1.0 - b_hat.sum()), float(np.sqrt(one @ C @ one)))
    if verbose:
        print(
            f"\n{'-'*78}\n  A) MASS-FRACTION sigma_beta  (analytic "
            f"(A^T W A)^-1)\n{'-'*78}"
        )
        print(f"  {'anomaly':14s} " + " ".join(f"{k:>16s}" for k in cases))
        for i, nm in enumerate(names):
            print(
                f"  {nm:14s} " + " ".join(f"{sig_by_case[k][i]:16.2e}" for k in cases)
            )
        print(f"  {'-'*76}")
        print(
            f"  {'BODY β̃':14s} "
        ...
        print(
            f"    (truth β̃ = {beta_bulk:.4f};  recovered "
        ...
        print(
            f"  {'geom. mean':14s} "
            + " ".join(
                f"{np.exp(np.mean(np.log(sig_by_case[k]))):16.2e}" for k in cases
            )
        )
        print(
            f"  {'WORST case':14s} "
    ...
    if do_position:
        ...
        if verbose:
            ...
            for i, nm in enumerate(names):
                print(
                    f"  {nm:14s} " + " ".join(f"{pos_rms[k][i]:16.2e}" for k in cases)
                )
            print(f"  {'-'*76}")
            print(
                f"  {'WORST case':14s} "
...
def robustness_sweep(
    ...
    for sd in seeds:
        ...
        for k in ratios:
            blocks = (
                base
                if k == "SH only"
                else base + P2.ch_blocks_for(P, nets[k], ch_modes, eps, f_true, bulk)
            )
            # analytic worst case: exact, and ~100x cheaper than 300 MC fits,
            # which is what makes a 24-interior sweep affordable
            wc[k] = np.sqrt(np.diag(np.linalg.inv(G.fisher_masses(blocks)))).max()
        for k in ratios:
    ...
    stab.update(
        {
            k: float(np.mean([len(a & v[0]) / n_cyl for a in v]))
    ...
    if verbose:
        ...
        print(
            f"  {'strategy':18s} {'median':>9s} {'min':>9s} {'max':>9s} "
            f"{'# draws better':>16s}"
        )
        for k, v in ratios.items():
            print(
                f"  {k:18s} {np.median(v):8.2f}× {v.min():8.2f}× {v.max():8.2f}× "
        ...
        for k in res["cases"]:
            if k in stab:
                tag = (
                    "   (fixed by construction: the score never sees the interior)"
                    if k in INTERIOR_INDEPENDENT
                    else ""
                )
                print(f"    {k:18s} {stab[k]:.2f}{tag}")
        print(
            "    ⇒ three of the four criteria never look at the interior at all, and "
...
def verdict(res, sweep=None, stability=None):
    ...
    print(
        f"  {'strategy':18s} {'geo-mean sig_b':>15s} {'worst sig_b':>13s} "
        f"{'vs FPS (worst)':>16s}"
    )
    for k in cases:
    ...
    print(
        ...
        f"({sig[ref].max()/sig[single].max():.2f}× vs farthest-point) — but this is"
        "\n  ONE draw, and the ratio spans roughly 0.5x to 4x across interiors, so"
        "\n  a single-interior ranking settles nothing.  The sweep below is the"
        "\n  result; this table is here to show where the numbers come from."
    )
    ...
    if sweep is not None:
        ...
        print(
            f"  Over {n_dr} independent interiors the best criterion is {best} at a"
            f"\n  MEDIAN {gain:.2f}× (winning {nb}/{n_dr} draws)"
            + (
                "."
                if survives
                else " — and the ratios straddle 1, so the\n  ranking flips from one interior "
                "to the next."
            )
        )
        ...
        if not survives:
            ...
            print(
                "      • and every criterion here correlates with radius at "
    ...
    if sweep is not None and gain >= 1.2 and nb >= 0.75 * n_dr:
        print(
            ...
            "     uncertainty by a further factor of roughly %.1f. That map is fixed\n"
            "     by the shape, the tracking geometry and the truncation degree, so it\n"
            '     is precomputable and requires no knowledge of the interior."' % gain
        )
    else:
        print(
            "\n  Suggested sentence:\n"
...
def make_plots(res, outdir="Images"):
    ...
    for pos, score, key, title in panels:
        ...
        a.scatter(
            mlon, mlat, marker="x", s=70, color=ACCENT, lw=2, label="mascon (sub-point)"
        )
        a.scatter(
            lon[fps],
            lat[fps],
    ...
    fig.suptitle(
        "Where should the cylinders go?  SH-aware criterion maps on the "
        "common candidate pool",
        fontsize=13,
    )
    fig.tight_layout()
    fig.savefig(
        os.path.join(outdir, "global_pt3_fig1_criteria.pdf"),
        dpi=180,
        bbox_inches="tight",
    )
    ...
    fig, axes = plt.subplots(
        1,
        ncol,
        figsize=(19 if has_sweep else 15.5, 5.8),
    ...
    a1.set_title(
        "Mass-fraction recovery per anomaly (sorted shallow to deep)\n"
    ...
    if has_sweep:  # the verdict panel: does any criterion survive averaging?
        ...
        for i, k in enumerate(keys):
            ...
            a3.scatter(
                np.full_like(v, i)
                + np.random.default_rng(i).uniform(-0.13, 0.13, len(v)),
                v,
                s=26,
                color=cols[cases.index(k)],
                edgecolor="k",
                lw=0.4,
                zorder=3,
            )
            a3.plot(
                [i - 0.28, i + 0.28], [np.median(v)] * 2, color="k", lw=2.2, zorder=4
            )
        a3.axhline(
            1.0, color=ACCENT, ls="--", lw=1.5, label="parity with farthest-point"
        )
        a3.set_yscale("log")
        ...
        a3.set_title(
            f"Across {len(sw[keys[0]])} independent interiors\n"
            "(bar = median; below the line = worse than geometry)"
        )
        a3.grid(True, axis="y", which="both", alpha=0.3)
        a3.legend(fontsize=8)
    fig.tight_layout()
    fig.savefig(
        os.path.join(outdir, "global_pt3_fig2_massratio.pdf"),
        dpi=180,
        bbox_inches="tight",
    )
    ...
    if res["pos_rms"]:
        ...
        ax.set_title(
            "Position recovery of randomly placed mascons: "
        ...
        fig.tight_layout()
        fig.savefig(
            os.path.join(outdir, "global_pt3_fig3_position.pdf"),
            dpi=180,
            bbox_inches="tight",
        )
    plt.show()
...
def _selftest(verbose=True):
    ...
    if verbose:
        print(
            f"  selftest OK — SH synthesis vs exact mascon field: "
            f"|dU|/U = {eU:.1e}, |dg|/g = {eg:.1e}"
        )
        print(
            f"  selftest OK — degree-only covariance ⇒ isotropic map: "
            f"sigma_U radial to {sp_pot:.1e} (exact), "
            f"sigma_g to {sp_grad:.1e} (central-difference limited)"
        )
        print(
            f"  selftest OK — constant-density bulk: mass = {c01[0]:.12f}, "
...
if __name__ == "__main__":
    ...
    res = run(
        ...
        n_mc_pos=30,
        do_position=True,
        n_sweep=24,  # the ratio spans ~0.5x-4x per interior; 8 cannot rank
        outdir="Images",
        verbose=True,
    )
    print(
