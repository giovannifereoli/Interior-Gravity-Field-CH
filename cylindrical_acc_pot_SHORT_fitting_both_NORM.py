...
BETA = 0.1  # Regularization parameter for the coefficients
    ...
    for m in range(n_m):
        for n in range(1, n_n + 1):
            ...
            for i in range(num_points):
                # Radial (rho)
                A[3 * i, idx] = norm * dV_drho[i] * cos_m_phi[i]
                A[3 * i, idx + 1] = norm * dV_drho[i] * sin_m_phi[i]

                # Angular (phi)
                A[3 * i + 1, idx] = norm * dV_dphi[i] * -sin_m_phi[i]
                A[3 * i + 1, idx + 1] = norm * dV_dphi[i] * cos_m_phi[i]

                # Axial (z)
                A[3 * i + 2, idx] = norm * dV_dz[i] * cos_m_phi[i]
                A[3 * i + 2, idx + 1] = norm * dV_dz[i] * sin_m_phi[i]
...
def prepare_linear_system_for_cylindrical_potential(points, potentials, n_n, n_m):
    ...
    for m in range(n_m):
        for n in range(1, n_n + 1):
            ...
            A[:, idx] = exp_term * bessel_j * np.cos(m * phi) * norm
            A[:, idx + 1] = exp_term * bessel_j * np.sin(m * phi) * norm
    ...
    for m in range(n_m):
        for n in range(1, n_n + 1):
            ...
            fitted_acceleration[:, 0] += (
                norm * dV_drho * fitted_params[idx] * cos_m_phi
                + norm * dV_drho * fitted_params[idx + 1] * sin_m_phi
            )
            fitted_acceleration[:, 1] += (
                norm * dV_dphi * fitted_params[idx] * -sin_m_phi
                + norm * dV_dphi * fitted_params[idx + 1] * cos_m_phi
            )
            fitted_acceleration[:, 2] += (
                norm * dV_dz * fitted_params[idx] * cos_m_phi
                + norm * dV_dz * fitted_params[idx + 1] * sin_m_phi
            )
