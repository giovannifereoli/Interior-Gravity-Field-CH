...
precomputed_bessel = {
    (m, n): (
        sp.jv(m, bessel_zeros[m][n - 1] / (ALPHA)),
...
def compute_integrand_A(z, phi, phi_alpha, j_mn):
    exp_value = np.exp(j_mn * z / (ALPHA * R_star))
    return phi_alpha * exp_value * np.cos(phi)


def compute_integrand_B(z, phi, phi_alpha, j_mn):
    exp_value = np.exp(j_mn * z / (ALPHA * R_star))
    return phi_alpha * exp_value * np.sin(phi)


# Function to compute A_mn and B_mn in parallel
def compute_coefficients(m, n):
    j_mn = bessel_zeros[m][n - 1]
    J_mn_noR, J_mn_squared = precomputed_bessel[(m, n)]

    def integrand_A(phi, z):  # OSS: contrary!
        phi_alpha = Phi_alpha(R_star, phi, z)
        return compute_integrand_A(z, phi, phi_alpha, j_mn)

    def integrand_B(phi, z):
        phi_alpha = Phi_alpha(R_star, phi, z)
        return compute_integrand_B(z, phi, phi_alpha, j_mn)
    ...
    A_mn = (1 / (np.pi * J_mn_noR)) * A_mn_integral
    B_mn = (1 / (np.pi * J_mn_noR)) * B_mn_integral
